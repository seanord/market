...
class Mouse_page(Base):
    ...
    page_title_value = "Мыши"  # ожидаемый заголовок раздела (часть)
    ...
    # тип подключения
    connection_type_expand = "//div[@data-id='f[558]']"
    ...
    # ссылка на раздел "аксессуары"
    accessories_link = "//span[@itemprop='name' and contains(.,'Периферия')]"
    ...
    # ссылка на корзину
    basket_link = "//span[@class='buttons__link-icon cart-link-counter__icon']"
    ...
    def get_accessories_link(self):
        return WebDriverWait(self.driver, 30, ignored_exceptions=self.ignored_exceptions).until(
            EC.element_to_be_clickable((By.XPATH, self.accessories_link)))
    ...
    def get_basket_link(self):
        return WebDriverWait(self.driver, 30, ignored_exceptions=self.ignored_exceptions).until(
            EC.element_to_be_clickable((By.XPATH, self.basket_link)))
    ...
    def select_wireless_connection(self):
        ...
        print("Выбираем беспроводные мыши")
    ...
    def buy_top_item(self):
        ...
        print("Покупаем первую мышь из списка")
    ...
    def click_accessories_link(self):
        action = ActionChains(self.driver)
        action.move_to_element(self.get_accessories_link()).perform()
        self.get_accessories_link().click()
        print('Возвращаемся в раздел "Периферия и аксессуары"')
    ...
    def click_basket_link(self):
        self.get_basket_link().click()
        print("Переходим в корзину")

    # def wait_until_basket_window_disappears(self):
    #     self.get_basket()
    #     print("Окошко корзины исчезло")
    ...
    def select_mouse(self, min_price, max_price, num):
